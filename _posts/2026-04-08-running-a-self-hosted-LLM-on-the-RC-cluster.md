...
The GPU we worked with was an Nvidia GTX 1080 Ti, a solid card when it came out in 2017, but GPUs have improved quite a bit since 2017!. The GTX 1080 Ti has 11GB VRAM which set a ceiling on what models were available to us.
...
Getting the model running was surprisingly straightforward. Ollama is a tool that manages and serves LLM models. On the cluster machine, I downloaded the Ollama binary, pulled the model, and started the server.
...
**ngrok** was the obvious first attempt. ngrok is a 3rd party service that tunnels traffic through their servers and gives you a public URL. It worked immediately, but the whole point of self-hosting was privacy, so routing every prompt and response through a third party defeated the purpose.
