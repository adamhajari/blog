...
The Recurse Center has a small computing cluster called the Heap which includes three Linux machines with GPUs. We wanted to see if we could turn one of these machines into a self-hosted LLM server that RC community members could use for privacy-sensitive tasks, like summarizing the daily check-in posts on our internal Zulip chat.

The GPU we worked with was an Nvidia GTX 1080 Ti. The 1080 Ti has 11GB of VRAM which was a solid card when it came out in 2017. Large language models need to fit in VRAM to run at reasonable speeds. At Q4_K_M quantization (a compression scheme that preserves roughly 85% of a model's original quality), the biggest models we could run were in the 12-14 billion parameter range. We used benchmarks from llmrun.dev to compare options and landed on Google's Gemma 3 12B: it uses about 7.9GB of VRAM and generates around 40 tokens per second on the 1080 Ti. Fast enough for conversation.

## Setup

Getting the model running was surprisingly straightforward. Ollama is an open source tool that downloads, manages, and serves LLM models. On the Heap machine, I downloaded the Ollama binary, pulled the model, and started the server. For a web interface, we ran Open WebUI, an open-source ChatGPT-style frontend. Within an hour we had a working chat interface talking to Gemma 3 on a GPU. 

The hard part was making it accessible to anyone else. The cluster sits behind a firewall and wasn't reachable from the outside. We needed a way to get traffic in.

**ngrok** was the obvious first attempt. Ngrok is a service that tunnels traffic through their servers and gives you a public URL. It worked immediately, but the whole point of self-hosting was privacy. Routing every prompt and response through a third party defeated that purpose.

We then turned to an nginx reverse proxy running on a Raspberry Pi at RC's office. RC has a community deployment tool called disco that lets Recursers deploy Docker apps to this Pi. We deployed a minimal nginx config that forwards all traffic to the Heap. The result was an HTTPS endpoint that proxies through the Pi to Open WebUI on the heap. All traffic stays on RC's infrastructure.
...
Gemma 4 e4b is a Mixture of Experts (MoE) model. It has many more total parameters than its name suggests, but only activates ~4 billion of them per token. This makes it significantly faster than the dense 31B model while producing similar quality output. For tasks like this, the speed-quality tradeoff favored e4b by a long shot. Unfortunately the GPU memory constraints on the cluster precluded both of the Gemma 4 models and we ended up going with the Gemma 3 model instead.

## A better choice: a modern Macbook

Here's the irony: in 2026, a base model MacBook Air has 16GB of unified memory available for GPU inference. That's roughly 25% more than the 1080 Ti's 11GB, with faster memory bandwidth. My 2024 MacBook Pro has 64GB of available memory opening up even more options. Running Ollama locally on a laptop means no network latency, no proxy chain, no firewall issues, and access to slightly larger models. For individual use, a modern laptop is likely the better option for running open-source models.
